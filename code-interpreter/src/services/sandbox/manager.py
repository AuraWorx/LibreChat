...
from .nsjail import NsjailConfig, SandboxInfo, SessionUidAllocator
...
class SandboxManager:
    ...
    def __init__(self):
        ...
        # One allocator per manager (per-process) so every live sandbox on
        # this host gets a distinct real uid. See nsjail.SessionUidAllocator.
        self._uid_allocator = SessionUidAllocator()
    ...
    def create_sandbox(
        ...
        info = SandboxInfo(
            ...
            repl_mode=repl_mode,
            labels=labels,
            outside_uid=self._uid_allocator.allocate(),
        )
    ...
    def destroy_sandbox(self, sandbox_info: SandboxInfo) -> bool:
        ...
        try:
            ...
            # Always release the uid back to the pool, even if directory
            # cleanup failed — an uncollected uid just shrinks the pool, but
            # this is the only place it's freed, so skipping it on error
            # would leak it permanently.
            if sandbox_info.outside_uid is not None:
                self._uid_allocator.release(sandbox_info.outside_uid)
